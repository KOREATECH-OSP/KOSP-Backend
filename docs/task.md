...
| 기능 ID | 소분류 | 기능명 | 상태 | 구현 상세 / 비고 |
|:---:|:---:|:---:|:---:|:---|
| **CLG-001** | 도전과제 | 깃허브 활동내역 조회/평가 | 🧪 예정 | GitHub API 활용 점수화 |
...
| **ADM-003** | 챌린지 | 챌린지 추가 | 🧪 예정 | 조건 파싱 로직 |
...
1.  **회원 탈퇴 (USR-002)**: 즉시 Soft Delete 처리 (유예 기간 X).
2.  **깃허브 연동 (USR-003, CLG-001)**: GitHub API 연동.

**Priority 4: Infrastructure / AI**
1.  **신고 기능 (TAK-004, ADM-009)**: 기획 확정 후 구현.
2.  **관리자 챌린지 관리 (ADM-003~005)**: 동적 조건 관리 시스템.
