package io.swkoreatech.kosp.domain.auth.api;
...
import io.swkoreatech.kosp.domain.auth.dto.request.EmailVerificationRequest;
import io.swkoreatech.kosp.domain.auth.dto.request.GithubTokenRequest;
import io.swkoreatech.kosp.domain.auth.dto.request.LoginRequest;
import io.swkoreatech.kosp.domain.auth.dto.request.PasswordResetRequest;
...
public interface AuthApi {
    ...
    @Operation(summary = "1. Github 토큰 교환 (회원가입용)", description = "Github Access Token을 검증하고 회원가입용 토큰(JWS)을 발급합니다.")
    ...
    @Operation(summary = "2. 학번/사번 중복 확인", description = "회원가입 시 학번 또는 사번의 중복 여부와 형식을 확인합니다. (학번: 10자리, 사번: 6/8자리)")
    @GetMapping("/verify/identity")
    ResponseEntity<CheckMemberIdResponse> checkMemberId(
        @Valid CheckMemberIdRequest request
    );

    @Operation(summary = "3. 이메일 인증 코드 발송", description = "회원가입을 위해 이메일로 인증 코드를 발송합니다.")
    ...
    @Operation(summary = "4. 이메일 인증 코드 검증", description = "이메일로 발송된 인증 코드를 검증합니다.")
    ...
    @Operation(summary = "5. 회원가입 토큰 검증", description = "회원가입 폼 진입 전 JWS 토큰의 유효성을 검증합니다. 위변조된 토큰은 접근을 차단합니다.")
    @GetMapping("/verify/token/signup")
    ResponseEntity<Void> validateSignupToken(
        @Parameter(description = "회원가입 토큰 (JWS)", required = true, hidden = true) @Token SignupToken token
    );

    @Operation(summary = "6. 일반 로그인", description = "이메일과 비밀번호로 로그인합니다. Access Token과 Refresh Token을 반환합니다.")
    @PostMapping("/login")
    ResponseEntity<AuthTokenResponse> login(
        @RequestBody @Valid LoginRequest request
    );

    @Operation(summary = "7. Github 로그인", description = "Github Access Token을 사용하여 로그인하고 Access Token과 Refresh Token을 발급합니다.")
    @PostMapping("/login/github")
    ResponseEntity<AuthTokenResponse> loginWithGithub(
        @Parameter(description = "Github Access Token") @RequestBody @Valid GithubTokenRequest request
    );

    @Operation(summary = "8. 내 정보 조회", description = "현재 로그인된 사용자의 기본 정보를 조회합니다.")
    @GetMapping("/me")
    ResponseEntity<AuthMeResponse> getMyInfo(
        @Parameter(hidden = true) User user
    );

    @Operation(summary = "9. 토큰 재발급", description = "Refresh Token을 사용하여 새로운 Access Token을 발급합니다.")
    @PostMapping("/reissue")
    ...
    @Operation(summary = "10. 로그인 토큰 검증", description = "Access Token의 유효성을 검증합니다. 만료되거나 위변조된 토큰은 차단합니다.")
    ...
    @Operation(summary = "11. 비밀번호 재설정 메일 발송", description = "비밀번호 재설정을 위해 이메일로 링크를 발송합니다.")
    ...
    @Operation(summary = "12. 비밀번호 재설정", description = "발송된 링크의 토큰을 사용하여 비밀번호를 재설정합니다.")
    @PostMapping("/reset/password/confirm")
    ResponseEntity<Void> resetPassword(
        @Parameter(description = "토큰 및 새 비밀번호") @RequestBody @Valid PasswordResetRequest request);

    @Operation(summary = "13. 로그아웃", description = "서버에서 Refresh Token을 삭제하여 로그아웃 처리합니다.")
    @PostMapping("/logout")
