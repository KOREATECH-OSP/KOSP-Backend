...
public class ScoreCalculationStep implements StepProvider {
    ...
    private int calculateRepoActivityScore(int commitCount, int prCount) {
        if (hasHighActivity(commitCount, prCount))
            return 3;
        if (hasMediumActivity(commitCount, prCount))
            return 2;
        if (hasLowActivity(commitCount, prCount))
            return 1;
        return 0;
    }
    ...
    private BigDecimal calculateImpactScoreInternal(List<ContributedRepoDocument> repos,
    ...
    private void saveScores(String githubId, BigDecimal activityScore, BigDecimal diversityScore,
